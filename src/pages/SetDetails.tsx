...
function SetDetails(props: SetDetailsProps) {
  ...
  useEffect(() => {
    if (
      ...
      Array.isArray(data) &&
      data[4] &&
      Array.isArray(data[4].result)
    ) {
      setTokenContracts(
        data[4].result.map((tokenAddress: string) => ({
          abi: Erc20,
  ...
  const {
    data: tokenNames,
    isLoading: tokenNamesLoading,
    isError: tokenNamesError,
  } = useReadContracts({
    ...
    <>
      <div className="verticalStack">
              ...
              {isLoading
                ...
                : isError
                  ...
                  : data
                    ? parseFloat(
                        formatUnits(
                          data[2].result as unknown as bigint,
                          data[3].result as unknown as number
                        )
                      ).toFixed(0)
                    : "N/A"}
            </span>
          ...
          <div className="propertyRow">
            ...
            <span className="propertyValue">
              {tokenNamesLoading
                ? "Loading token names..."
                : tokenNamesError
                  ? "Error loading token names"
                  : tokenNames &&
                    tokenNames.map((name) => name.result || "N/A").join(", ")}
            </span>
          </div>
        </div>
        ...
        <div className="buttonContainer">
          <button className="issueButton" onClick={() => handleInitialize()}>
            Initialize
          </button>
          <button
          ...
          <button className="issueButton" onClick={() => handleIssue()}>
            Issue
          </button>
          <button className="redeemButton" onClick={() => handleRedeem()}>
            Redeem
          </button>
        </div>
      </div>
